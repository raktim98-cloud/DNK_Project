...
const Card = ({ title, description, buttonText, imageUrl }) => {
  ...
  return (
    ...
    className="relative rounded shadow-lg ">
      <div className="w-auto h-auto md:h-[481px]">
      ...
      <div>
        <div className="px-6  absolute left-0 bottom-0  -translate-y-7">
          <div className=" text-white font-bold text-xl mb-2">{title}</div>
          <p className=" text-white  text-lg pb-6">
            {description}
          </p>
          
          <button className="w-full md:w-40 bg-white transition-all duration-500 hover:bg-black text-black hover:text-white font-bold py-2.5 px-5">
            {buttonText}
          </button>
